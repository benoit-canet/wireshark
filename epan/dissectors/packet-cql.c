...
static int hf_cql_v3_long_string_length = -1;
...
static int hf_cql_v3_string__result_rows_global_table_spec_ksname = -1;
static int hf_cql_v3_string__result_rows_global_table_spec_table_name = -1;
static int hf_cql_v3_string__result_rows_table_name = -1;
static int hf_cql_v3_string__result_rows_keyspace_name = -1;
static int hf_cql_v3_string__result_rows_column_name = -1;
...
dissect_cql_v3_query_parameters(proto_tree* cql_subtree, tvbuff_t* tvb, gint offset, int execute)
{
	...
	/* flags */
	proto_tree_add_item_ret_uint(cql_subtree, hf_cql_v3_flags, tvb, offset, 1, ENC_BIG_ENDIAN, &flags);
	proto_tree_add_item(cql_subtree, hf_cql_v3_query_flags_values, tvb, offset, 1, ENC_BIG_ENDIAN);
	proto_tree_add_item(cql_subtree, hf_cql_v3_query_flags_skip_metadata, tvb, offset, 1, ENC_BIG_ENDIAN);
	proto_tree_add_item(cql_subtree, hf_cql_v3_query_flags_page_size, tvb, offset, 1, ENC_BIG_ENDIAN);
	proto_tree_add_item(cql_subtree, hf_cql_v3_query_flags_paging_state, tvb, offset, 1, ENC_BIG_ENDIAN);
	proto_tree_add_item(cql_subtree, hf_cql_v3_query_flags_serial_consistency, tvb, offset, 1, ENC_BIG_ENDIAN);
	proto_tree_add_item(cql_subtree, hf_cql_v3_query_flags_default_timestamp, tvb, offset, 1, ENC_BIG_ENDIAN);
	proto_tree_add_item(cql_subtree, hf_cql_v3_query_flags_names_for_values, tvb, offset, 1, ENC_BIG_ENDIAN);
	offset += 1;
...
cql_v3_transaction_type* cql_transaction_add_request(cql_v3_conversation_type* conv,
                                                     packet_info* pinfo,
                                                     gint32 stream,
                                                     int fake)
{
...
cql_v3_transaction_type* cql_enrich_transaction_with_response(cql_v3_conversation_type* conv,
                                                              packet_info* pinfo,
                                                              gint32 stream)
{
	cql_v3_transaction_type* trans;
...
cql_v3_transaction_type* cql_transaction_lookup(cql_v3_conversation_type* conv,
                                                packet_info* pinfo,
                                                gint32 stream)
{
    wmem_list_frame_t* frame;
...
dissect_cql_v3_tcp_pdu(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree, void* data _U_)
{
	...
	/* Early bail out checks */
	if(tvb_reported_length_remaining(tvb, offset) < 9)
		/* Must have at least 9 bytes for a CQL v3 frame header */
		return -1;
	...
	offset += 1;
	proto_tree_add_item(cql_tree, hf_cql_v3_flags, tvb, offset, 1, ENC_BIG_ENDIAN);
	proto_tree_add_item(cql_tree, hf_cql_v3_flag_compression, tvb, offset, 1, ENC_BIG_ENDIAN);
	proto_tree_add_item(cql_tree, hf_cql_v3_flag_tracing, tvb, offset, 1, ENC_BIG_ENDIAN);
	offset += 1;
	...
	if (server_to_client == 0) {
		switch (opcode) {
			...
			case CQL_V3_OPCODE_QUERY:
				...
				/* Query */
				proto_tree_add_item_ret_uint(cql_subtree, hf_cql_v3_long_string_length, tvb, offset, 4, ENC_BIG_ENDIAN, &string_length);
				offset += 4;
				proto_tree_add_item(cql_subtree, hf_cql_v3_string, tvb, offset, string_length, ENC_UTF_8 | ENC_NA);
				offset += string_length;
			...
			case CQL_V3_OPCODE_EXECUTE:
				...
				/* Query parameters */
				offset = dissect_cql_v3_query_parameters(cql_subtree, tvb, offset, 1);
				break;
				...
				switch (result_kind) {
					...
					case CQL_V3_RESULT_KIND_ROWS:
						...
						if (result_rows_flags & CQL_V3_RESULT_ROWS_FLAG_GLOBAL_TABLES_SPEC) {
							...
							offset += 2;
							proto_tree_add_item(metadata_subtree, hf_cql_v3_string__result_rows_global_table_spec_ksname, tvb, offset, string_length, ENC_UTF_8 | ENC_NA);
							offset += string_length;

							proto_tree_add_item_ret_uint(metadata_subtree, hf_cql_v3_string_length, tvb, offset, 2, ENC_BIG_ENDIAN, &string_length);
							offset += 2;
							proto_tree_add_item(metadata_subtree, hf_cql_v3_string__result_rows_global_table_spec_table_name, tvb, offset, string_length, ENC_UTF_8 | ENC_NA);
							offset += string_length;
						}
						...
						if (result_rows_flags & CQL_V3_RESULT_ROWS_FLAG_NO_METADATA) {
							...
							for (j = 0; j < result_rows_columns_count; ++j) {
								if (!result_rows_flags & CQL_V3_RESULT_ROWS_FLAG_GLOBAL_TABLES_SPEC) {
									...
									proto_tree_add_item_ret_uint(metadata_subtree, hf_cql_v3_string_length, tvb, offset, 2, ENC_BIG_ENDIAN, &string_length);
									offset += 2;
									proto_tree_add_item(metadata_subtree, hf_cql_v3_string__result_rows_keyspace_name, tvb, offset, string_length, ENC_UTF_8 | ENC_NA);
									offset += string_length;
									proto_tree_add_item_ret_uint(metadata_subtree, hf_cql_v3_string_length, tvb, offset, 2, ENC_BIG_ENDIAN, &string_length);
									offset += 2;
									proto_tree_add_item(metadata_subtree, hf_cql_v3_string__result_rows_table_name, tvb, offset, string_length, ENC_UTF_8 | ENC_NA);
									offset += string_length;
								}
								...
								proto_tree_add_item_ret_uint(metadata_subtree, hf_cql_v3_string_length, tvb, offset, 2, ENC_BIG_ENDIAN, &string_length);
								offset += 2;
								proto_tree_add_item(metadata_subtree, hf_cql_v3_string__result_rows_column_name, tvb, offset, string_length, ENC_UTF_8 | ENC_NA);
								offset += string_length;
...
static int
dissect_cql(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree, void* data)
{
	if (pinfo->ptype == PT_TCP) {
		return dissect_cql_v3_tcp(tvb, pinfo, tree, data);
	} else {
		return -1;
	}
}
...
proto_reg_handoff_cql(void)
{
	...
	/*TODO: static int current_port;*/

	if (!initialized) {
		cql_handle = create_dissector_handle(dissect_cql, proto_cql);
		initialized = TRUE;
	} else {
		dissector_delete_uint("tcp.port", CQL_DEFAULT_PORT, cql_handle);
	}
...
proto_register_cql(void)
{
	static hf_register_info hf[] = {
		...
		{
			&hf_cql_v3_string__result_rows_global_table_spec_ksname,
			{
		...
		{
			&hf_cql_v3_string__result_rows_global_table_spec_table_name,
			{
		...
		{
			&hf_cql_v3_string__result_rows_table_name,
			{
		...
		{
			&hf_cql_v3_string__result_rows_keyspace_name,
			{
		...
		{
			&hf_cql_v3_string__result_rows_column_name,
			{
		...
		{
			&hf_cql_v3_long_string_length,
			{
				"Long String Length", "cql.string_length",
				FT_UINT32, BASE_DEC,
				NULL, 0x0,
				"Length of the subsequent long string value.", HFILL
			}
		},
		{
	...
	proto_cql = proto_register_protocol(
